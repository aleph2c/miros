...
if __name__ == '__main__':
  ...
  archer.cancel_events(Event(signal=signals.Second))
  archer.live_trace = False
  others = list(archer.others.keys())
  first = others[0]
  last = others[-1]
  print(archer.others[last].trace())
  print(archer.others[first].trace())

  archer.others[first].live_spy = True
  time.sleep(1.0)
  # confirm paths in empathy chart
  # confirm not_waiting paths
  archer.post_fifo(Event(signal=signals.Other_Advance_War_Cry, payload=first))
  archer.post_fifo(Event(signal=signals.Other_Skirmish_War_Cry, payload=first))
  archer.post_fifo(Event(signal=signals.Other_Retreat_War_Cry, payload=first))

  # confirm path to dead
  archer.post_fifo(Event(signal=signals.Advance_War_Cry, payload=first))
  archer.post_fifo(Event(signal=signals.Other_Advance_War_Cry, payload=first))
  archer.post_fifo(Event(signal=signals.Retreat_War_Cry, payload=first))

  # confirm path to waiting
  archer.post_fifo(Event(signal=signals.Other_Retreat_Ready_War_Cry, payload=first))
  archer.post_fifo(Event(signal=signals.Other_Ready_War_Cry, payload=first))
