...
* posting an ``e2`` event which while S1 Region is in its final state and the S2
  region is in it's ``S22`` state.  This ``e2`` event will cause the S2 Region
...
The code used to start the chart, and it's regions will be written as a method
...
If you would like to compare the diagram to the code, the code listing is
...
tell it's components what event to fire when they are done.   It has an
...
We call our main chart the ``XMLChart`` because we are deriving it's behaviour
...
Any flat method decorated with the ``@instrumented`` will route it's
...
.. code-block:: python
   ...
   if __name__ == '__main__':
     ...
     # lines 8-26 in log file
     example.post_fifo(Event(signal=signals.to_p))

     # lines 27-38 in log file
