...
Our story will take place in a little universe. This little universe will
...
underworld will be ruled by Theo, `"the solipsist" <https://en.wikipedia.org/wiki/Solipsism>`_. The earth
...
.. admonition:: Translation
  ...
  all of its calls to the various state functions will be driven by this
  thread.
...
have its own heaven and underworld, gods and explorer spirit, but its
...
.. admonition:: Translation
  ...
  doesn't handle an event in any of its callback functions, the event will be
  ignored.
...
take out its scroll.  Then he will lean across the bar and whisper the answer
into Tara's ear.
...
.. admonition:: Translation
  ...
  developer placed code between the "Merve" clause and its return statement,
...
.. admonition:: Translation
  ...
  from its current state, to the LCA.  However, it should not exit the LCA.
...
**have meaning**.  So, they create an organized campaign: "hack
...
To help us, they create a `Rosetta stone <https://www.dictionary.com/browse/rosetta-stone>`_, translating the concepts of their universe into something legible for you and me:
...
there is no information about the thread, **S**,
**T**, the deques, the events or any of the dynamics of the
statechart.
...
invent a `spy-carpet <https://www.aetv.com/real-crime/a-surveillance-expert-on-planting-bugs-in-carpets-cats-and-cockroaches-to-nab-suspects>`_.
To use this carpet, you place the **@spy_on** decorator above
...
transpired between **T**, **S** and any human within that pub.  This information can be read during or after their universehas reacted to the events send from our world.
...
I will break its design process up into 6 steps, or iterations.
...
the :ref:`boiler plate example <recipes-boiler-plate-state-method>`, and change it to match your design.
...
Your oven thread starts up its event processor, attaches to your callback
...
situated, **S** and **T** in the off state, it checks its queue to see if
anything is there.
...
``start_at`` call, its queue will look like this:
...
its work.  It still needs to process the "Baking" and "Off" events.
...
If your statechart receives an event that is not handled within its source state,
...
its plan would be made up of a list of functions that need to be exited.
...
the entry conditions to re-join **T**; its first step will trigger the entry
...
* **S** enters the toasting state by sending its callback an ENTRY_SIGNAL
  event.
...
There is no if-elif clause in the ``off`` state callback, so its else clause is
triggered:
...
value of the ``trans`` method into its status variable and returns whatever
...
Next, **S** begins moving to rejoin **T**.  Its first step is to call the exit
...
.. note::
  ...
  There is only one handled init signal in the whole design and its in the
...
back to its queue to see if any other thread posted event's to it while it was
...
If this were a real toaster oven, its state would occur in two different
...
"heater_on" to print and the enters the baking state, causing its entry
...
working.  Not even close. We will get serious about testing in the next iteration.
...
* :ref:`Why isn't the deep history handled by the framework? <zero_to_one-why-isnt-the-deep-history-handled-by-the-framework3>`
...
through all of its state transitions.  Then we can look at the
...
leave as a regression test. A better testing approach is :ref:`demonstrated in the next iteration. <iter4_proof>`
...
.. _zero_to_one-why-isnt-the-deep-history-handled-by-the-framework3:
...
includes its own event loop and software bus.  It's really cool.
...
its hardware dependent calls have been mocked.  That makes the code portable,
...
* :ref:`The buzz seems like a pointless feature, what's going on? <zero_to_one-the-buzz-seems-like-a-pointless-feature-whats-going-on4>`
...
* :ref:`Why do you break the transition-tests apart from the other tests? <zero_to_one-why-do-you-break-transition-tests-apart-from-other-test>`
* :ref:`Why don't you just copy out your spy output and use it as the test target? <zero_to_one-why-dont-you-just-copy-out-your-spy-output-and-use-it-as-the-test-target>`
* :ref:`Where did you get your trace-test-target, and what is going on with that stripped call? <zero_to_one-where-did-you-get-your-trace-test-target-and-what-is-going-on-with-that-stripped-call>`
* :ref:`Why don't you just copy out your trace output and use it as a test target? <zero_to_one-why-dont-you-just-copy-out-your-trace-output-and-use-it-as-a-test-target>`
* :ref:`Your light_on, light_off ... tests seem pretty light, are you sure you are testing these features? <zero_to_one-your-light_on-light_off-tests-seem-pretty-light-are-you-sure-you-are-testing-these-features>`
...
prints ``buzz`` (11) then calls the ``oven.buzz()`` code of its derived
...
While Tara and Spike were spending time together on an
...
.. _zero_to_one-the-buzz-seems-like-a-pointless-feature-whats-going-on4:
...
.. _zero_to_one-why-do-you-break-transition-tests-apart-from-other-test:
...
.. _zero_to_one-why-dont-you-just-copy-out-your-spy-output-and-use-it-as-the-test-target:
...
.. _zero_to_one-where-did-you-get-your-trace-test-target-and-what-is-going-on-with-that-stripped-call:
...
.. _zero_to_one-why-dont-you-just-copy-out-your-trace-output-and-use-it-as-a-test-target:
...
.. _zero_to_one-your-light_on-light_off-tests-seem-pretty-light-are-you-sure-you-are-testing-these-features:
...
"""""""""""""""""""""
* :ref:`What happens if they don't take out there food after a buzz rings? <zero_to_one-what-happens-if-they-dont-take-out-there-food-after-a-buzz-rings5>`
...
* :ref:`Why is your tolerance of jitter so high? <zero_to_one-why-is-your-tolerance-of-jitter-so-high5>`
* :ref:`How can I reduce the jitter? <zero_to_one-how-can-i-reduce-the-jitter5>`


.. _zero_to_one-what-happens-if-they-dont-take-out-there-food-after-a-buzz-rings5:
...
''''''''''''''''''''''''''''''''''''''''''''''''
The deferred one shot pushes a behavior into the future.  its event will arrive
...
``cancel_events`` call, that universe is annihilated before its event can be
presented Theo.
...
.. _zero_to_one-why-is-your-tolerance-of-jitter-so-high5:
...
delay. Our customers won't notice if they have to wait an additional 5 ms to hear their
toaster oven buzz.
...
can just drop into that part of the system and look at its picture to
...
buzzes used to tell someone their food is done, from 2 to 3. Now I have to
remember to go back into the timing diagram and mess about with the drawn
pulses, I have to re-size the picture and make sure all of the words fit.
I'm not adding a lot of value here and this work would slow me down.
...
''''''''''''''''''''''''''''''''''''''''''''''
Nametuples are immutable and they are easy to make and describe; they make
great payload data structures. You don't have to use a namedtuple; but I
advise that you use something immutable.
