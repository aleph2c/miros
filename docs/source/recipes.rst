...
These sections contain examples that can be referenced as you are
building up your own statechart programs.
...
* It accepts two arguments:
  ...
  #. an object of type/subclass of ``miros.ActiveObject``, which has an event processor.
  #. an event of type/subclass of  ``miros.Event``

* It describes how it is situated in a hierarchy and how it is connected to
  other states, by changing the ``temp.fun`` attribute of its first argument.
...
to search the hierarchical state machine, to run its entry and exit conditions
...
event the same way.  They set the ``temp.fun`` to their super state (its outer
...
figure out, who was the super state of ``inner_state`` so it could call its
...
Hook code can be run when you sent an event to the state or any of its
...
Since each ``ActiveObject`` runs in its own thread they will process their
...
its capabilities <thread_safe_attributes-thread-safe-attributes>`, but it will
...
The f2 inner_state doesn't write to the ``times_in_inner`` property, so its
...
.. _recipes-boiler-plate-state-method:
...
The ``self.outer_state`` method does not describe its parent, this is
...
.. code-block:: python
  ...
  class ThreadSafeAttributesInActiveObject(ThreadSafeAttributes, ActiveObject):
   ...
   if __name__ == '__main__':
      ...
      ao.start_at(c)
      # Change the ActiveObject's attributes while it is starting its thread
...
specific class has.  You don't care about its internal events and you don't
...
So in many ways UML has become a kind of anti-brand but it has its good parts.
